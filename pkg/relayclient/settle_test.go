...
func TestVerifyDeliver_HashMismatch_FailsLoud(t *testing.T) {
	...
	_, _, err := verifyDeliver(context.Background(), nil, nil, ev, 0, nil)
	if err == nil {
...
func TestVerifyDeliver_Match_ReturnsContent(t *testing.T) {
	...
	got, gotHash, err := verifyDeliver(context.Background(), nil, nil, ev, 0, nil)
	if err != nil {
...
func TestVerifyDeliver_Oversize_FailsLoud(t *testing.T) {
	...
	_, _, err := verifyDeliver(context.Background(), nil, nil, ev, 10, nil) // ceiling far below the 200-byte body
	if err == nil {
