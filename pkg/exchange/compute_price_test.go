...
// used as the acquisition-scale base when present (post-put-accept).
// PutPrice=1000, fresh entry, rep=50 (default) -> repFactor=1.0.
// All other multipliers=1.0. acquisition-scale = 1000 * 1.2 = 1200; delivery
// (dontguess-af3/96e: amortized over resaleAmortizationN=4) = 1200 / 4 = 300.
func TestComputePrice_BasePrice_PutPriceUsedWhenPresent(t *testing.T) {
	...
	price := eng.ComputePriceForTest(entry)
	if price != 300 {
		t.Errorf("computePrice(PutPrice=1000, fresh entry, default rep) = %d, want 300", price)
	}
}

// TestComputePrice_BasePrice_TokenCostFallback verifies the pre-accept proxy:
// TokenCost * 0.7 when PutPrice=0. TokenCost=1000, rep=50 -> acquisition-scale
// price=700; delivery (amortized /4) = 175.
func TestComputePrice_BasePrice_TokenCostFallback(t *testing.T) {
	...
	price := eng.ComputePriceForTest(entry)
	if price != 175 {
		t.Errorf("computePrice(TokenCost=1000, PutPrice=0) = %d, want 175", price)
	}
}
...
// PutPrice=1000 -> acquisition-scale=1200. Stale (>=60d) -> ageFactor=0.5 ->
// 600; delivery (amortized /4) = 150.
func TestComputePrice_AgeFactor_StaleEntryLowerPrice(t *testing.T) {
	...
	if stalePrice != 150 {
		t.Errorf("computePrice(stale 90d, PutPrice=1000) = %d, want 150 (0.5x ageFactor, amortized /4)", stalePrice)
	}
}
...
func TestComputePrice_AgeFactor_ZeroTimestampNoDecay(t *testing.T) {
	...
	// PutTimestamp=0 -> ageFactor=1.0 -> acquisition-scale=1200, amortized
	// (/4) = 300 (same as no-decay baseline).
	if zeroPrice != 300 {
		t.Errorf("PutTimestamp=0 price=%d, want 300 (no decay)", zeroPrice)
	}
}
...
// PutPrice=1000, size=100KB: sizeBonus=0.30 (cap) -> sizeFactor=1.30 ->
// acquisition-scale=1560; delivery (amortized /4) = 390.
func TestComputePrice_ContentSize_LargerContentHigherPrice(t *testing.T) {
	...
	if largePrice != 390 {
		t.Errorf("computePrice(PutPrice=1000, ContentSize=100KB) = %d, want 390", largePrice)
	}
}
...
// (rep defaults to DefaultReputation=50 -> repFactor=1.0). Acquisition-scale=1200;
// delivery (dontguess-af3/96e, amortized over resaleAmortizationN=4) = 300.
func TestComputePrice_AllSignals_FreshEntryBaseline(t *testing.T) {
	...
	price := eng.ComputePriceForTest(entry)
	if price != 300 {
		t.Errorf("computePrice(baseline) = %d, want 300", price)
	}
}
...
// PutPrice=1000 -> acquisition-scale=1200 (1.2x margin). Tier=hot -> 1.2 * 1.5
// = 1800; delivery (amortized /4, dontguess-af3/96e) = 450.
func TestComputePrice_Tier_HotPremium(t *testing.T) {
	...
	price := eng.ComputePriceForTest(entry)
	if price != 450 {
		t.Errorf("computePrice(PutPrice=1000, tier=hot) = %d, want 450 (1.2x margin * 1.5x tier / 4x amortization)", price)
	}
}

// TestComputePrice_Tier_WarmPremium verifies that a "warm" entry commands a 1.2x
// tier multiplier on top of the base price.
// PutPrice=1000 -> acquisition-scale=1200. Tier=warm -> 1200 * 1.2 = 1440;
// delivery (amortized /4) = 360.
func TestComputePrice_Tier_WarmPremium(t *testing.T) {
	...
	price := eng.ComputePriceForTest(entry)
	if price != 360 {
		t.Errorf("computePrice(PutPrice=1000, tier=warm) = %d, want 360 (1.2x margin * 1.2x tier / 4x amortization)", price)
	}
}

// TestComputePrice_Tier_ColdNoChange verifies that a "cold" entry has no tier
// premium (1.0x multiplier — same as unset tier).
// PutPrice=1000 -> acquisition-scale=1200. Tier=cold -> 1200 * 1.0 = 1200;
// delivery (amortized /4) = 300.
func TestComputePrice_Tier_ColdNoChange(t *testing.T) {
	...
	price := eng.ComputePriceForTest(entry)
	if price != 300 {
		t.Errorf("computePrice(PutPrice=1000, tier=cold) = %d, want 300 (no tier premium)", price)
	}
}

// TestComputePrice_Tier_UnsetNoChange verifies that an unset tier ("") has no
// premium — same price as a "cold" entry.
// PutPrice=1000 -> acquisition-scale=1200. Tier="" -> 1200 * 1.0 = 1200;
// delivery (amortized /4) = 300.
func TestComputePrice_Tier_UnsetNoChange(t *testing.T) {
	...
	price := eng.ComputePriceForTest(entry)
	if price != 300 {
		t.Errorf("computePrice(PutPrice=1000, tier=\"\") = %d, want 300 (unset tier = no premium)", price)
	}
}
...
// TokenCost=1000 -> acquisition-scale=700 (0.7x seller share). hot=1050,
// warm=840, cold=700; delivery (amortized /4, dontguess-af3/96e) below.
func TestComputePrice_Tier_SameTokenCostDifferentTiersProduceDifferentPrices(t *testing.T) {
	...
	// Exact values: TokenCost=1000, PutPrice=0 -> acquisition-scale=700 (0.7x).
	// rep=50 -> repFactor=1.0. hot: 700*1.5=1050 /4=262.5->263. warm: 700*1.2=840
	// /4=210. cold: 700*1.0=700 /4=175.
	if hotPrice != 263 {
		t.Errorf("hot price = %d, want 263 (700 * 1.5 / 4)", hotPrice)
	}
	if warmPrice != 210 {
		t.Errorf("warm price = %d, want 210 (700 * 1.2 / 4)", warmPrice)
	}
	if coldPrice != 175 {
		t.Errorf("cold price = %d, want 175 (700 * 1.0 / 4)", coldPrice)
	}
}
