...
**Permanent constraint — no forward secrecy, retroactive blast radius:** both the wrap envelopes and the ciphertext live forever on an append-only relay (and Blossom). A single `operatorKey` compromise decrypts **all content ever put**; rotation gives no retroactive protection because old envelopes are immutable. MEMORY notes the operator key lives as a file in `~/.dontguess` and that a prior infra GC already overwrote a campfire key — so this is a live custody risk, not theoretical. Mitigations to ship: HSM/1Password custody, optional operator epoch subkeys (accepting entries expire with their epoch — which fights the publisher model's indefinite re-sale, a tension stated openly), and an explicit threat-model line: *on operator-key leak, the entire historical corpus is decryptable offline from scraped relay+Blossom data.* This pushes any operator-blind upgrade up the priority list — but see §4.3 for why "blind" is not actually reachable.
